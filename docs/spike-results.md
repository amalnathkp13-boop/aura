...
- **Deployment decision:** python daemons (ear/brain/guardian/face) run under **systemd** as root from `~/aura-src`; the matrix face ships as a small **Arduino App** (`board-app/`: app.yaml + sketch with `Bridge.provide("state", ...)` + a python main that tails `~/.aura/state.json` + `alerts.jsonl` and `Bridge.call`s 4 bytes `[presence, motion, activity, alert]` at 5 Hz — reusing the wire semantics of `aura/face/bridge.py`, which remains the PC-testable reference implementation).
