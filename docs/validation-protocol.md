...
Then restart the brain so it loads the new thresholds: `ssh arduino@192.168.63.60 "sudo systemctl restart aura-brain" < /dev/null`. Note: a calibration.json created before the RuView upgrade has no `rv` thresholds - re-run Learn my room once after deploying.
