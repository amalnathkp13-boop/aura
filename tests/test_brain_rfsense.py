...
def test_default_detector_is_rfsense(tmp_path, monkeypatch):
    monkeypatch.setenv("AURA_HOME", str(tmp_path))
    cfg = Config.load()
    assert cfg.detector == "rfsense"
...
def test_brain_writes_rfsense_detail(tmp_path, monkeypatch):
    monkeypatch.setenv("AURA_HOME", str(tmp_path))
    cfg = Config.load()
    frames_path = tmp_path / "frames.jsonl"
    _write_live(frames_path, 120, jitter=4.0)
    run_brain(cfg, frames_path, threading.Event(), model_path=None, max_iters=1)
    d = json.loads((tmp_path / "sense.json").read_text())
    assert d["state"]["src"] == "rfsense"
    assert len(d["links"]) >= 1 and "vote" in d["links"][0]
    assert "ts" in d and "present_frac" in d


def test_detector_cnn_without_model_falls_back_to_rfsense(tmp_path, monkeypatch):
    monkeypatch.setenv("AURA_HOME", str(tmp_path))
    ...
    _write_live(frames_path, 120, jitter=4.0)
    run_brain(cfg, frames_path, threading.Event(), model_path=None, max_iters=1)
    assert json.loads((tmp_path / "state.json").read_text())["src"] == "rfsense"
